...
*Date Preference:* ${selectedDate || "Upcoming Weekend Departure"}
*Sharing Plan:* ${sharingPlan.toUpperCase()} Sharing
*Number of Guests:* ${guestsCount}
*Quotation:* Custom Package Quotation Requested
...
Please share the official package quotation and confirm my seat reservation!`;
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-2 sm:p-4 bg-slate-950/80 backdrop-blur-md overflow-y-auto animate-fade-in">
      <div className="relative w-full max-w-2xl bg-white rounded-3xl shadow-2xl overflow-hidden border border-slate-100 my-auto max-h-[94vh] flex flex-col">
        ...
        <div className="relative bg-gradient-to-r from-[#074125] to-[#042c19] text-white p-5 sm:p-7 shrink-0">
          <button
            onClick={onClose}
          ...
          <div className="inline-flex items-center gap-2 px-3 py-0.5 rounded-full bg-[#059669]/20 border border-[#059669]/40 text-[#34d399] text-[10px] sm:text-xs font-black uppercase tracking-wider mb-2">
        ...
        {submitted ? (
          ...
          <div className="p-8 sm:p-12 text-center flex flex-col items-center justify-center space-y-4">
            <div className="w-16 h-16 rounded-full bg-[#ecfdf5] text-[#059669] flex items-center justify-center mb-2 animate-bounce">
              <CheckCircle className="w-10 h-10" />
            </div>
            <h4 className="text-xl sm:text-2xl font-black text-[#074125]">
              Reservation Request Sent!
            </h4>
            <p className="text-xs sm:text-sm text-slate-600 max-w-md">
              Thank you <strong className="text-slate-900">{fullName}</strong>! We have forwarded your booking inquiry directly to our official WhatsApp reservation desk.
            </p>
          ...
          <form onSubmit={handleSubmit} className="p-4 sm:p-7 space-y-5 overflow-y-auto flex-1">
            {/* Step 1: Sharing Plan Selector or Day Expedition */}
            {tour.nights > 0 ? (
              <div>
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
                  {[
                    { key: "quad", label: "Quad (4-5 Pax)", desc: "Standard Sharing" },
                    { key: "triple", label: "Triple (3 Pax)", desc: "3 In 1 Room" },
                    { key: "twin", label: "Twin (2 Pax)", desc: "2 In 1 Room" },
                    { key: "privateCouple", label: "Private Couple", desc: "Couple Suite" },
                  ].map((plan) => (
                    <button
                      key={plan.key}
                      type="button"
                    ...
                    >
                      ...
                      <div className="text-[11px] font-bold text-[#059669] mt-0.5 truncate">
                        {plan.desc}
                      </div>
                    </button>
                  ))}
              ...
              <div>
                ...
                <div className="p-3 rounded-2xl bg-[#ecfdf5] border border-[#059669]/20 text-xs font-bold text-[#074125] flex items-center gap-2">
                  <Sparkles className="w-4 h-4 text-[#059669] shrink-0" />
                  <span>Day Tour — Same Day Return (Standard Seat)</span>
                </div>
              </div>
            )}
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
              <div>
                <label className="block text-[11px] font-black uppercase tracking-wider text-slate-700 mb-1">
                  2. Departure Preference
                </label>
                <div className="relative">
                  <select
                    value={selectedDate}
                    onChange={(e) => setSelectedDate(e.target.value)}
                    className="w-full pl-9 pr-3 py-2.5 rounded-xl border border-slate-200 focus:border-[#059669] text-xs sm:text-sm font-bold text-slate-800 bg-white cursor-pointer"
                  >
                    <option value="Upcoming Friday Night Departure">Upcoming Friday Night Departure</option>
                    <option value="Upcoming Thursday Night Departure">Upcoming Thursday Night Departure</option>
                    <option value="Next Week Departure">Next Week Departure</option>
            ...
            <div className="space-y-3 pt-2 border-t border-slate-100">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <input
                  ...
                  className="w-full px-3.5 py-2.5 rounded-xl border border-slate-200 focus:border-[#059669] text-xs sm:text-sm font-medium"
                />
                <input
                  type="tel"
                  required
            ...
            <div className="bg-[#042c19] text-white p-4 sm:p-5 rounded-2xl flex flex-col sm:flex-row items-center justify-between gap-3 sm:gap-4 shadow-xl">
              <div className="w-full sm:w-auto text-center sm:text-left">
                ...
                <div className="text-[10px] text-[#34d399] font-semibold">
              ...
              <button
                type="submit"
                className="w-full sm:w-auto px-6 py-3.5 rounded-full bg-[#059669] hover:bg-[#047857] text-white font-black text-xs sm:text-sm shadow-[0_4px_18px_rgba(5,150,105,0.4)] transition-all flex items-center justify-center gap-2 transform active:scale-95"
              >
