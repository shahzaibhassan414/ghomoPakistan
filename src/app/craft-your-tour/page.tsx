...
export default function CraftYourTourPage() {
  ...
  const [vehicle, setVehicle] = useState<string>("Grand Cabin Van (10-12 Pax)");
  ...
  const destinationOptions = [
    ...
    { name: "Astore & Minimarg", icon: "🌸" },
    { name: "Sharan Forest & Shogran", icon: "🌲" },
    { name: "Muskhpuri Top", icon: "⛰️" },
  ];
    ...
    <div className="bg-[#f8fafc] min-h-screen pt-24 sm:pt-28 pb-20 sm:pb-24">
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        <form onSubmit={handleSubmit} className="bg-white rounded-3xl p-5 sm:p-10 shadow-xl border border-slate-200/80 space-y-6 sm:space-y-8">
          ...
          <div>
            <label className="block text-[11px] sm:text-xs font-black uppercase tracking-wider text-[#074125] mb-2.5 flex items-center gap-2">
              <span className="w-5 h-5 sm:w-6 sm:h-6 rounded-full bg-[#059669] text-white flex items-center justify-center text-[10px] sm:text-xs font-bold shrink-0">1</span>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-5 sm:gap-6">
            <div>
              <label className="block text-[11px] sm:text-xs font-black uppercase tracking-wider text-[#074125] mb-2 flex items-center gap-2">
                <span className="w-5 h-5 sm:w-6 sm:h-6 rounded-full bg-[#059669] text-white flex items-center justify-center text-[10px] sm:text-xs font-bold shrink-0">2</span>
                <span>Trip Category</span>
              </label>
              <select
                value={tripType}
                onChange={(e) => setTripType(e.target.value)}
                className="w-full px-3.5 py-3 rounded-2xl border border-slate-200 focus:border-[#059669] text-xs sm:text-sm font-bold text-slate-800 bg-white"
              >
            ...
            <div>
              <label className="block text-[11px] sm:text-xs font-black uppercase tracking-wider text-[#074125] mb-2 flex items-center justify-between">
                <span>Trip Duration</span>
                <span className="text-[#059669] font-black">{tripDuration} Days / {tripDuration - 1} Nights</span>
              </label>
              <input
                type="range"
                min={2}
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-5 sm:gap-6">
            <div>
              <label className="block text-[11px] sm:text-xs font-black uppercase tracking-wider text-[#074125] mb-2 flex items-center gap-2">
                <span className="w-5 h-5 sm:w-6 sm:h-6 rounded-full bg-[#059669] text-white flex items-center justify-center text-[10px] sm:text-xs font-bold shrink-0">3</span>
              ...
              <div className="grid grid-cols-3 gap-2">
                {[
                  ...
                  { key: "luxury", label: "Executive", desc: "5-Star Resort" },
                ].map((tier) => (
                  <button
                    key={tier.key}
                    type="button"
                    ...
                    className={`p-2.5 sm:p-3 rounded-2xl border text-left transition-all ${
                      travelStyle === tier.key
                        ? "border-[#059669] bg-[#ecfdf5] text-[#074125] ring-2 ring-[#059669]/30"
            ...
            <div>
              <label className="block text-[11px] sm:text-xs font-black uppercase tracking-wider text-[#074125] mb-2">
                Preferred Vehicle
              </label>
              <select
                value={vehicle}
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3.5 sm:gap-4">
            <div>
              <label className="block text-[11px] sm:text-xs font-black uppercase tracking-wider text-[#074125] mb-1.5 flex items-center gap-2">
                <span className="w-5 h-5 rounded-full bg-[#059669] text-white flex items-center justify-center text-[10px] font-bold shrink-0">4</span>
              ...
              <input
                ...
                className="w-full px-3.5 py-2.5 rounded-xl border border-slate-200 focus:border-[#059669] text-xs sm:text-sm font-bold text-slate-800"
              />
            </div>

            <div>
              <label className="block text-[11px] sm:text-xs font-black uppercase tracking-wider text-[#074125] mb-1.5">
                Departure City
              </label>
              <select
                value={departureCity}
                onChange={(e) => setDepartureCity(e.target.value)}
                className="w-full px-3 py-2.5 rounded-xl border border-slate-200 focus:border-[#059669] text-xs sm:text-sm font-medium bg-white"
              >
            ...
            <div>
              ...
              <input
                ...
                className="w-full px-3 py-2.5 rounded-xl border border-slate-200 focus:border-[#059669] text-xs sm:text-sm font-medium"
              />
            </div>
          </div>
          ...
          <div className="pt-3 border-t border-slate-200/80 space-y-3 sm:space-y-4">
            <label className="block text-[11px] sm:text-xs font-black uppercase tracking-wider text-[#074125]">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
              <input
                ...
                className="w-full px-3.5 py-2.5 sm:py-3 rounded-xl border border-slate-200 focus:border-[#059669] text-xs sm:text-sm font-medium"
              />
              <input
                type="tel"
                required
            ...
            <textarea
              ...
              className="w-full px-3.5 py-2 rounded-xl border border-slate-200 focus:border-[#059669] text-xs sm:text-sm font-medium"
            />
          </div>

          {/* Custom Quote & WhatsApp Submission */}
          <div className="bg-[#042c19] text-white p-5 sm:p-8 rounded-3xl flex flex-col sm:flex-row items-center justify-between gap-5 sm:gap-6 shadow-2xl">
            <div className="text-center sm:text-left">
              <span className="text-[10px] sm:text-xs text-[#34d399] font-bold uppercase tracking-wider">Bespoke Itinerary:</span>
            ...
            <button
              type="submit"
              className="w-full sm:w-auto px-6 sm:px-8 py-3.5 sm:py-4 rounded-full bg-[#059669] hover:bg-[#047857] text-white font-black text-xs sm:text-sm shadow-[0_4px_20px_rgba(5,150,105,0.4)] transition-all flex items-center justify-center gap-2 transform active:scale-95 whitespace-nowrap"
            >
